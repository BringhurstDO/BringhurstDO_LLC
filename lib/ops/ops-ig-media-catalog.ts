import "server-only";

import type { OpsProjectId } from "@/lib/ops/types";

export type OpsIgMediaCatalogEntry = {
  assetLocation: string;
  id: string;
  label: string;
  projectId: OpsProjectId;
  tags: string[];
};

const BUILTIN_CATALOG: OpsIgMediaCatalogEntry[] = [
  {
    assetLocation: "/ops-ig/america-250-square.png",
    id: "america-250-square",
    label: "America 250 — Instagram square",
    projectId: "bringhurstdo",
    tags: ["america-250", "brand", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/america-250-facebook.png",
    id: "america-250-facebook",
    label: "America 250 — Facebook link",
    projectId: "bringhurstdo",
    tags: ["america-250", "brand", "approved", "facebook"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-product-screenshot.png",
    id: "syncsoap-product-screenshot",
    label: "SyncSOAP product screenshot",
    projectId: "syncsoap",
    tags: ["product", "screenshot", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-product-cover-landscape.png",
    id: "syncsoap-product-cover-landscape",
    label: "SyncSOAP product cover — landscape",
    projectId: "syncsoap",
    tags: ["product", "screenshot", "approved", "facebook", "x", "linkedin"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-product-cover-square.png",
    id: "syncsoap-product-cover-square",
    label: "SyncSOAP product cover — square",
    projectId: "syncsoap",
    tags: ["product", "screenshot", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-product-screenshot-ig-square.png",
    id: "syncsoap-product-screenshot-ig-square",
    label: "SyncSOAP product — IG square",
    projectId: "syncsoap",
    tags: ["product", "screenshot", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-progress-ig-01-cover.png",
    id: "syncsoap-progress-ig-01-cover",
    label: "SyncSOAP progress carousel — cover",
    projectId: "syncsoap",
    tags: ["syncsoap", "progress", "carousel", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-progress-ig-02-product.png",
    id: "syncsoap-progress-ig-02-product",
    label: "SyncSOAP progress carousel — product",
    projectId: "syncsoap",
    tags: ["syncsoap", "progress", "carousel", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-progress-ig-03-study.png",
    id: "syncsoap-progress-ig-03-study",
    label: "SyncSOAP progress carousel — study",
    projectId: "syncsoap",
    tags: ["syncsoap", "progress", "carousel", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-progress-ig-04-vision.png",
    id: "syncsoap-progress-ig-04-vision",
    label: "SyncSOAP progress carousel — vision",
    projectId: "syncsoap",
    tags: ["syncsoap", "progress", "carousel", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-progress-ig-05-lesion.png",
    id: "syncsoap-progress-ig-05-lesion",
    label: "SyncSOAP progress carousel — lesion assistant",
    projectId: "syncsoap",
    tags: ["syncsoap", "progress", "carousel", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-progress-ig-06-ai.png",
    id: "syncsoap-progress-ig-06-ai",
    label: "SyncSOAP progress carousel — AI governance",
    projectId: "syncsoap",
    tags: ["syncsoap", "progress", "carousel", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-progress-ig-07-security.png",
    id: "syncsoap-progress-ig-07-security",
    label: "SyncSOAP progress carousel — security",
    projectId: "syncsoap",
    tags: ["syncsoap", "progress", "carousel", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-progress-ig-08-evidence.png",
    id: "syncsoap-progress-ig-08-evidence",
    label: "SyncSOAP progress carousel — evidence",
    projectId: "syncsoap",
    tags: ["syncsoap", "progress", "carousel", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-progress-ig-09-deploy.png",
    id: "syncsoap-progress-ig-09-deploy",
    label: "SyncSOAP progress carousel — deployment",
    projectId: "syncsoap",
    tags: ["syncsoap", "progress", "carousel", "approved", "instagram"],
  },
  {
    assetLocation: "/ops-ig/syncsoap-progress-ig-10-summary.png",
    id: "syncsoap-progress-ig-10-summary",
    label: "SyncSOAP progress carousel — summary",
    projectId: "syncsoap",
    tags: ["syncsoap", "progress", "carousel", "approved", "instagram"],
  },
  {
    assetLocation: "/syncsafety-mock.jpg",
    id: "syncsafety-product-mock",
    label: "SyncSafety product mock (legacy)",
    projectId: "syncsafety",
    tags: ["product", "screenshot", "approved"],
  },
  {
    assetLocation: "/ops-ig/syncsafety-product-cover-landscape.png",
    id: "syncsafety-product-cover-landscape",
    label: "SyncSafety product cover — landscape",
    projectId: "syncsafety",
    tags: ["product", "screenshot", "approved", "facebook", "x", "linkedin"],
  },
  {
    assetLocation: "/ops-ig/syncsafety-product-cover-square.png",
    id: "syncsafety-product-cover-square",
    label: "SyncSafety product cover — square",
    projectId: "syncsafety",
    tags: ["product", "screenshot", "approved", "instagram"],
  },
  {
    assetLocation: "/bringhurstdo-social-header.png",
    id: "bringhurstdo-social-header",
    label: "BringhurstDO social header (legacy)",
    projectId: "bringhurstdo",
    tags: ["brand", "approved"],
  },
  {
    assetLocation: "/ops-ig/bringhurstdo-product-cover-landscape.png",
    id: "bringhurstdo-product-cover-landscape",
    label: "BringhurstDO product cover — landscape",
    projectId: "bringhurstdo",
    tags: ["product", "brand", "approved", "facebook", "x", "linkedin"],
  },
  {
    assetLocation: "/ops-ig/bringhurstdo-product-cover-square.png",
    id: "bringhurstdo-product-cover-square",
    label: "BringhurstDO product cover — square",
    projectId: "bringhurstdo",
    tags: ["product", "brand", "approved", "instagram"],
  },
  {
    assetLocation: "/founder-headshot.jpg",
    id: "founder-headshot",
    label: "Founder headshot",
    projectId: "bringhurstdo",
    tags: ["founder", "approved"],
  },
];

function parseEnvCatalog() {
  const raw = process.env.OPS_IG_MEDIA_CATALOG?.trim();

  if (!raw) {
    return [] as OpsIgMediaCatalogEntry[];
  }

  try {
    const parsed = JSON.parse(raw) as unknown;

    if (!Array.isArray(parsed)) {
      return [];
    }

    const entries: OpsIgMediaCatalogEntry[] = [];

    for (const item of parsed) {
      if (!item || typeof item !== "object") {
        continue;
      }

      const record = item as {
        assetLocation?: unknown;
        id?: unknown;
        label?: unknown;
        projectId?: unknown;
        tags?: unknown;
      };
      const assetLocation =
        typeof record.assetLocation === "string" ? record.assetLocation.trim() : "";
      const id = typeof record.id === "string" ? record.id.trim() : "";
      const label = typeof record.label === "string" ? record.label.trim() : "";
      const projectId =
        record.projectId === "syncsoap" ||
        record.projectId === "syncsafety" ||
        record.projectId === "bringhurstdo"
          ? record.projectId
          : null;

      if (!assetLocation || !id || !label || !projectId) {
        continue;
      }

      entries.push({
        assetLocation,
        id,
        label,
        projectId,
        tags: Array.isArray(record.tags)
          ? record.tags.filter((tag): tag is string => typeof tag === "string")
          : [],
      });
    }

    return entries;
  } catch {
    return [];
  }
}

export function listOpsIgMediaCatalog(projectId?: OpsProjectId) {
  const merged = new Map<string, OpsIgMediaCatalogEntry>();

  for (const entry of [...BUILTIN_CATALOG, ...parseEnvCatalog()]) {
    merged.set(entry.id, entry);
  }

  const catalog = Array.from(merged.values()).sort((left, right) =>
    left.label.localeCompare(right.label),
  );

  if (!projectId) {
    return catalog;
  }

  return catalog.filter((entry) => entry.projectId === projectId);
}
